...
/** Неблокирующая синхронная пауза (не грузит CPU) */
...
class ScheduleManager {
    ...
    /** Время последнего изменения файла (для оптимизации автообновления) */
    getMtime() {
    ...
    createEmpty() {
        ...
        const ws2 = XLSX.utils.aoa_to_sheet([['Сотрудник', 'Дата', 'Начало', 'Конец', 'Обед', 'Ставка', 'ЗП/час', 'Надбавка', 'Надбавка тип']]);
        ws2['!cols'] = [{ wch: 18 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 8 }, { wch: 10 }, { wch: 10 }, { wch: 12 }];
    ...
    read() {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            const locked = emp1 === '🔒' || emp2 === '🔒';
            result.push({ date: dateStr, emp1, emp2, locked,
                status: filled === 0 ? 'свободно' : filled < this.maxPerDate ? 'частично' : 'занято', row: i });
        }
        return result;
    }

    /* Блокировка даты (без привязки к сотруднику) */
    lockDate(dateStr) {
        const wb = this._open();
        ...
        if (rowIdx === -1) {
            data.push([dateStr, '🔒', '', 'занято']);
        } else {
            const row = data[rowIdx]; while (row.length < 4) row.push('');
            const e1 = String(row[1] || '').trim(), e2 = String(row[2] || '').trim();
            if (e1 === '🔒' || e2 === '🔒') return { success: false, message: 'Дата уже заблокирована' };
            if (!e1) row[1] = '🔒'; else if (!e2) row[2] = '🔒'; else return { success: false, message: 'Дата уже заполнена' };
            const f = (row[1] ? 1 : 0) + (row[2] ? 1 : 0);
            row[3] = f >= this.maxPerDate ? 'занято' : 'частично';
        }
        this._saveSheet(wb, sn, data, [{ wch: 14 }, { wch: 18 }, { wch: 18 }, { wch: 14 }]);
        return { success: true, message: 'Дата заблокирована: ' + dateStr };
    }
    ...
    unlockDate(dateStr) {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (String(row[1]).trim() === '🔒') { row[1] = ''; found = true; }
            if (String(row[2]).trim() === '🔒') { row[2] = ''; found = true; }
            if (found) {
                const f = (row[1] ? 1 : 0) + (row[2] ? 1 : 0);
        ...
        if (!found) return { success: false, message: 'Блокировка не найдена' };
        this._saveSheet(wb, sn, data, [{ wch: 14 }, { wch: 18 }, { wch: 18 }, { wch: 14 }]);
        return { success: true, message: 'Блокировка снята: ' + dateStr };
    }
    ...
    readWork() {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            res.push({ emp: String(r[0]).trim(), date: this._dateVal(r[1]),
                ...
                hourlyWage: parseFloat(r[6]) || 0, allowance: parseFloat(r[7]) || 0,
                allowanceType: String(r[8] || '').trim() });
        }
        return res;
    }

    setWork(emp, dateStr, start, end, lunch, rate, hourlyWage, allowance, allowanceType) {
        const wb = this._open();
        this._ensureWithMigration(wb, 'Работа', ['Сотрудник', 'Дата', 'Начало', 'Конец', 'Обед', 'Ставка', 'ЗП/час', 'Надбавка', 'Надбавка тип']);
        const data = XLSX.utils.sheet_to_json(wb.Sheets['Работа'], { header: 1 });
        let found = false;
        for (let i = 1; i < data.length; i++) {
            if (!data[i]) continue;
            if (String(data[i][0] || '').trim() === emp && this._dateVal(data[i][1]) === dateStr) {
                data[i] = [emp, dateStr, start, end, lunch, rate, hourlyWage || 0, allowance || 0, allowanceType || '']; found = true; break;
            }
        }
        if (!found) data.push([emp, dateStr, start, end, lunch, rate, hourlyWage || 0, allowance || 0, allowanceType || '']);
        this._saveSheet(wb, 'Работа', data, [{ wch: 18 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 8 }, { wch: 10 }, { wch: 10 }, { wch: 12 }]);
        return { success: true };
    }
    ...
    removeWork(emp, dateStr) {
        ...
        this._saveSheet(wb, 'Работа', nw, [{ wch: 18 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 8 }, { wch: 10 }, { wch: 10 }, { wch: 12 }]);
        return { success: ok };
    }
    ...
    readVacation() {
        ...
        for (let i = 1; i < data.length; i++) {
            const r = data[i]; if (!r || !r[0]) continue;
            res.push({ emp: String(r[0]).trim(), date: this._dateVal(r[1]), status: String(r[2] || 'approved').trim() });
        }
        return res;
    }

    addVacation(emp, dateStr, status) {
        const wb = this._open();
        this._ensure(wb, 'Отпуск', ['Сотрудник', 'Дата', 'Статус']);
        const data = XLSX.utils.sheet_to_json(wb.Sheets['Отпуск'], { header: 1 });
        for (let i = 1; i < data.length; i++) {
            if (!data[i]) continue;
            if (String(data[i][0] || '').trim() === emp && this._dateVal(data[i][1]) === dateStr)
        ...
        data.push([emp, dateStr, status || 'approved']);
        this._saveSheet(wb, 'Отпуск', data, [{ wch: 18 }, { wch: 14 }, { wch: 12 }]);
        return { success: true, message: 'Отпускной день добавлен' };
    }

    approveVacation(emp, dateStr) {
        const wb = this._open();
    ...
    rejectVacation(emp, dateStr) {
        ...
        for (let i = 1; i < data.length; i++) {
            if (!data[i]) continue;
            if (String(data[i][0] || '').trim() === emp && this._dateVal(data[i][1]) === dateStr) {
                if (String(data[i][2] || '').trim() === 'pending') { ok = true; continue; }
            }
            nw.push(data[i]);
        }
        this._saveSheet(wb, 'Отпуск', nw, [{ wch: 18 }, { wch: 14 }, { wch: 12 }]);
        return { success: ok, message: ok ? 'Отпуск отклонён' : 'Запись не найдена или уже утверждена' };
    }
    ...
    getEmployees() {
        const s = new Set();
        this.read().forEach(r => { if (r.emp1 && r.emp1 !== '🔒') s.add(r.emp1); if (r.emp2 && r.emp2 !== '🔒') s.add(r.emp2); });
    ...
    exportStructured(exportPath, breakType, tabMap) {
        ...
        allRows.forEach(r => {
            ...
            if (this.isOds) {
                ...
                aoa.push([pernr, dateObj, dateObj, begTime, endTime, breakType || 'F130']);
            } else {
        ...
        vacRows.forEach(r => {
            ...
            if (this.isOds) {
                const [dd, mm, yy] = r.date.split('.').map(Number);
                const dateObj = new Date(Date.UTC(yy, mm - 1, dd));
                vacAoa.push([pernr, dateObj, dateObj]);
            } else {
        ...
        tripRows.forEach(r => {
            ...
            if (this.isOds) {
                const [dd, mm, yy] = r.date.split('.').map(Number);
                const dateObj = new Date(Date.UTC(yy, mm - 1, dd));
                tripAoa.push([pernr, dateObj, dateObj]);
            } else {
    ...
    _open() {
        ...
        const wb = retry(() => XLSX.readFile(this.filePath));
    ...
    _ensureWithMigration(wb, name, fullHeaders) {
        if (!wb.Sheets[name]) {
            const ws = XLSX.utils.aoa_to_sheet([fullHeaders]);
            XLSX.utils.book_append_sheet(wb, ws, name);
            return;
        }
        const data = XLSX.utils.sheet_to_json(wb.Sheets[name], { header: 1 });
        if (data.length && data[0] && data[0].length < fullHeaders.length) {
            data[0] = fullHeaders;
            for (let i = 1; i < data.length; i++) {
                if (!data[i]) data[i] = [];
                while (data[i].length < fullHeaders.length) data[i].push('');
            }
    ...
    _write(wb) {
        ...
        retry(() => XLSX.writeFile(wb, tmp, { bookType: this.isOds ? 'ods' : 'xlsx' }));
        retry(() => fs.renameSync(tmp, this.filePath));
        try { this._cacheMtime = fs.statSync(this.filePath).mtimeMs; this._cache = wb; } catch (_) {}
    }
